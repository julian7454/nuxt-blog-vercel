<template>
    <header class="flex w-full justify-center">
        <nav class="flex w-full max-w-7xl items-center justify-between px-6 py-2">
            ...
            <NuxtLink v-if="!userInfo" class="px-3 py-2 text-gray-700 transition hover:text-emerald-500" to="/login">
                登入
            </NuxtLink>
            <div v-if="userInfo" class="group relative">
                <label for="avatar" class="cursor-pointer">
                    <img class="inline-block h-10 w-10 rounded-full bg-white/90 object-cover object-center p-0.5 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75"
                        :src="userInfo.avatar" alt="使用者選單" />
                </label>
                    ...
                    <div
                        ...
                        <div class="group/menu-item px-1 py-1">
                            <NuxtLink
                                ...
                                <Icon class="mr-2 h-5 w-5 text-emerald-400 group-hover/menu-item:text-white"
                                    name="ri:pencil-line" />
                                撰寫文章
                            </NuxtLink>
                        </div>
